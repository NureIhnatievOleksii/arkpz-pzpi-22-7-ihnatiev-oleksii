...
namespace AirSense.Api.Controllers
{
    ...
    public class AirQualityController(IMediator mediator, MqttService mqttService) : ControllerBase
    {
        ...
        {
            if (string.IsNullOrEmpty(mqttService.LastMessage))
            {
                return NotFound("Немає отриманих повідомлень.");
            }

            return Ok(mqttService.LastMessage);
        }
